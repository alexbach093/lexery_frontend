...
export function SettingsCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-[10px] border-none bg-transparent text-[#171717] transition-colors hover:bg-[#F4F4F4] active:bg-[#F4F4F4]"
    >
...
export function SettingsTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'flex min-h-10 w-[calc(100%-8px)] cursor-pointer items-center gap-2.5 self-center rounded-xl border-none px-3 text-left text-[#171717] transition-colors active:bg-[#F4F4F4]',
        active ? 'bg-[#F4F4F4]' : 'bg-transparent hover:bg-[#F4F4F4]'
      )}
    >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        disabled
          ? 'cursor-default border-[#E4E4E4] bg-[#F4F4F4] text-[#AAAAAA]'
          : danger
            ? 'cursor-pointer border-[#F19A9A] bg-white text-[#F25555] hover:bg-[#FFF0F0]'
            : 'cursor-pointer border-[#D7D7D7] bg-white text-[#3A3A3A] hover:bg-[#F4F4F4]'
      )}
    >
      {children}
    </button>
